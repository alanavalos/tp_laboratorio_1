 ...
 *
 */
int al_add(ArrayList* pList,void* pElement)//agrega al final
{
    int returnAux = -1;
    void* auxiliar = NULL;
    int auxControl;
...
//si es = a null devuelve -1 sino el largo del array
...
void* al_get(ArrayList* pList , int index)
{
    ...
    if(pList!=NULL&&index>=0&&index<=pList->size)
    {
        ...
        //devuelve un puntero a void, por ende se le agrega el *
    }
...
int al_clear(ArrayList* pList)
{
    ...
    {
        free(pList->pElements);
        pList->size=0;// y que ponga el largo nuevamente en 0
        returnAux=0;
    }
...
int al_push(ArrayList* pList, int index, void* pElement)
{
    ...
    if(pList!=NULL&&pElement!=NULL)
    {
        ...
        {
            expand(pList,index);
            al_set(pList,index,pElement);//agrega en determianda posocion
            pList->size++;
...
int al_isEmpty(ArrayList* pList)
{
    ...
    if(pList!=NULL)
    {
       if(pList->size==0)//cantidad de elememtos
       {
           returnAux=1;
...
int al_sort(ArrayList* pList, int (*pFunc)(void* ,void*), int order)
{
    ...
    if(pList!=NULL&&pFunc!=NULL&&order>=0&&order<=1)
    {
        ...
        for(i=0; i<pList->size-1; i++)
        {
            ...
            {
                orderAux=pFunc(*((pList->pElements)+i),*((pList->pElements)+j));
                if(order==0)
                {
                    if(orderAux==-1)
                    {
    ...
    if(pList!=NULL)
     {
         ...
         {
             auxiliar =(void*)realloc(pList->pElements,(sizeof(void*)* (pList->size + AL_INCREMENT)));
              if(auxiliar!=NULL)
              { returnAux=0;
